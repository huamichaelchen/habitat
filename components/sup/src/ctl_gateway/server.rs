    ...
    fn handshake(&self, socket: SrvStream) -> Box<Future<Item = SrvStream, Error = HandlerError>> {
        ...
        let handshake = socket
                    ...
                    move |m| {
                        ...
                        match m.parse::<protocols::ctl::Handshake>() {
                            Ok(decoded) => {
                                ...
                                Ok((m, decoded.get_secret_key() == secret_key, io))
                            }
                            Err(err) => {
            ...
            .and_then(|(msg, success, socket)| {
                ...
                socket.send(reply).map_err(HandlerError::from).and_then(
                    move |io| {
                        Ok((io, success))
                    },
                )
        ...
        Box::new(handshake.select2(self.timeout(REQ_TIMEOUT)).then(|res| {
            match res {
                ...
                Ok(Either::A(((_, false), _to))) => {
                    future::err(HandlerError::from(io::Error::new(
